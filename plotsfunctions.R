  ...
  for(i in 1:nfused){
    ...
    if(input_selectdist=="Density"){
      if(input_selectscale=="Absolute"){
        ...
        #legend("topright",cex=0.6,paste(c("freq (2.5 %):","freq (50 %):","freq (97.5 %):"),round(quantile(100*p0[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%") )) 
    ...
    if(input_selectdist=="Cumulative"){
      ...
      if(input_selectscale=="Absolute"){
        ...
        #legend("bottomright",inset=0.04,cex=0.6,paste(c("freq (2.5 %):","freq (50 %):","freq (97.5 %):"),round(quantile(100*p0[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%")  )) 
        ...
        if(nexactK[hazardindexK[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          if(input_selectdist=="Density"){ 
            ...
            if(input_selectscale=="Absolute"){
              ...
              #                     round(c(quantile(qutotal95,0.05,names=FALSE),quantile(qutotal95,0.5,names=FALSE),quantile(qutotal95,0.95,names=FALSE)),3)))
            ...
            #legend("bottomright",cex=0.6,paste(c("expo (2.5 %):","expo (50 %):","expo (97.5 %)"),round(quantile(100*PK[foodindex[i],hazardindexK[h]]*pK[,hazardindexK[h],foodindex[i]]*p0[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%")  ))
          } # end of if density
          ...
          if(input_selectdist=="Cumulative"){
            ...
            if(input_selectscale=="Logarithmic"){
              ...
              for(resample in 1:40){
                ...
                samplecon <- sample(samplecon,10000,replace=TRUE)
                samplecon0 <- sample(samplecon0,10000,replace=TRUE)
                lines(ecdf(log(sampleOIM*mean(samplecon)*RK[foodindex[i],hazardindexK[h]])/log(10)),verticals=TRUE,do.points=FALSE,xlim=c(minn/log(10),maxx/log(10)),lwd=1,lty=3,col="#D0006F")
                lines(ecdf(log(sampleOIM*mean(samplecon0)*RK[foodindex[i],hazardindexK[h]])/log(10)),verticals=TRUE,do.points=FALSE,xlim=c(minn/log(10),maxx/log(10)),lwd=1,lty=3,col="#004F71")
              }
        ...
        if(nexactM[hazardindexM[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          if(input_selectdist=="Density"){
            ...
            if(input_selectscale=="Absolute"){   
              ...
              for(u in 1:n_sim){ 
                ...
                qupos95[u] <- qlnorm(0.95,logRM[foodindex[i],hazardindexM[h]]
                                     +mus0[u,foodindex[i]]
                                     +mucM[u,hazardindexM[h],foodindex[i]]
                                     +muw[u],
                                     sqrt(Vs[u]
              ...
              # legend("right",cex=0.6,
              #        legend=paste(c("95% exposure (5 %):","95% exposure (50 %):","95% exposure (95 %):"),
              #        round(c(quantile(qutotal95,0.05,names=FALSE),
              #                quantile(qutotal95,0.5,names=FALSE),
              #                quantile(qutotal95,0.95,names=FALSE)))))
              
              # positive acute (poisson mean) exposures, 
              # posterior quantiles of 95% variability quantile:
              qu95_05 <- round(quantile(qupos95,0.05,names=FALSE,na.rm=TRUE))
              qu95_50 <- round(quantile(qupos95,0.5,names=FALSE,na.rm=TRUE))
              qu95_95 <- round(quantile(qupos95,0.95,names=FALSE,na.rm=TRUE))
              
              #legend("topright",cex=0.6,paste(c("95% exposure+ (5 %):","95% exposure+ (50 %):","95% exposure+ (95 %):"),c(qu95_05,qu95_50,qu95_95)))
            ...
            #legend("bottomright",cex=0.6,paste(c("expo (2.5 %):","expo (50 %):","expo (97.5 %):"),round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*p0[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%")  ))
          } # end of if density
          ...
          if(input_selectdist=="Cumulative"){
            ...
            if(input_selectscale=="Absolute"){
              ...
              # legend("topright",cex=0.6,paste(c("95% exposure+ (5 %):","95% exposure+ (50 %):","95% exposure+ (95 %):"),
              #                                 c(qu95_05,qu95_50,qu95_95)))
            ...
            if(input_selectscale=="Logarithmic"){
              ...
              for(resample in 1:40){
                ...
                samplecon <- sample(samplecon,10000,replace=TRUE)
                samplecon0 <- sample(samplecon0,10000,replace=TRUE)
                lines(ecdf(log(sampleser*samplecon*RM[foodindex[i],hazardindexM[h]])/log(10)),verticals=TRUE,do.points=FALSE,xlim=c(minn/log(10),maxx/log(10)),lwd=1,lty=3,col="#D0006F")
                lines(ecdf(log(sampleser*samplecon0*RM[foodindex[i],hazardindexM[h]])/log(10)),verticals=TRUE,do.points=FALSE,xlim=c(minn/log(10),maxx/log(10)),lwd=1,lty=3,col="#004F71")
              }
              lines(logmeanexposure/log(10),cump,main=paste(hazardnamesusedM[h],"from",foodnamesused[i],"(acute)"),xlab="log (A.exposure+)",ylab="",xlim=c(minn/log(10),maxx/log(10)),lwd=3) 
            } # end of if logarithmic
            #legend("bottomright",cex=0.6,paste(c("expo (2.5 %):","expo (50 %):","expo (97.5 %):"),round(quantile(100*PM[foodindex[i],hazardindexM[h]]*pM[,hazardindexM[h],foodindex[i]]*p0[,foodindex[i]],c(0.025,0.5,0.975),names=FALSE),2),c("%","%","%")  ))
      ...
      if(input_modelchoice == "Independent days"){
        ...
        for(h in 1:nhusedK){
          ...
          if(input_selectscale=="Absolute"){
            ...
            #legend("topright",title=paste("Total",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(Q[,h],c(0.05,0.5,0.95),names=FALSE),2) )) 
            #legend("bottomright",title=paste("Positives",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(Qplus[,h],c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE),2) ))
          ...
          if(input_selectscale=="Logarithmic"){
            ...
            # legend("topright",title=paste("Total",theQ,"% quantile:"),
            #        paste(c("5 %:","50 %:","95 %:"),
            #        round(quantile(log10(Q[,h]),c(0.05,0.5,0.95),names=FALSE),2) )) 
            # legend("bottomright",title=paste("Positives",theQ,"% quantile:"),
            #        paste(c("5 %:","50 %:","95 %:"),
            #        round(quantile(log10(Qplus[,h]),c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE),2) ))
      ...
      if(input_modelchoice == "Dependent days"){
        ...
        for(h in 1:nhusedK){
          ...
          if(input_selectscale=="Logarithmic"){
            ...
            #legend("topright",title=paste("Total",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(log(Q[,h])/log(10),c(0.05,0.5,0.95),names=FALSE),2) ))
            #legend("bottomright",title=paste("Positives",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(log10(Qplus[,h]),c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE),2) ))
    ...
    if((nhusedM>0)&(nfused>0)){  
        ...
        for(u in 1:nU){ # for nU parameter sets
          ...
          wmc[1:nV] <- rlnorm(nV,muw[mc[u]],sigw[mc[u]]) # bodyweight for v:th individual
            if(nf>1){ # if many foods
              ...
              #for(f in 1:nf){   # error 1:nf!
              #smc[1:nV,f] <- exp(rmvnorm(nV,musmc[1:nV,f],Ss[mc[u],f,f]))  # actual amount
              #}
            for(v in 1:nV){  
        ...
        for(h in 1:nhusedM){
          ...
          if(input_selectscale=="Absolute"){
            ...
            if(sum(!is.na(exposurevarsample[1,]))>0){
              ...
              #xmax <- quantile(Qplus[,h],0.95,names=FALSE,na.rm=TRUE)
            ...
            #legend("topright",title=paste("Total",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(Q[,h],c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE)) ))
            #legend("bottomright",title=paste("Positives",theQ,"% quantile:"),
            #        paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(Qplus[,h],c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE)) ))
          ...
          if(input_selectscale=="Logarithmic"){
            ...
            if(sum(!is.na(exposurevarsample[1,]))>0){
              ...
              #xmax <- log10(quantile(Qplus[,h],0.95,names=FALSE,na.rm=TRUE)) 
            ...
            #legend("topright",title=paste("Total",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(log10(Q[,h]),c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE),2) ))
            #legend("bottomright",title=paste("Positives",theQ,"% quantile:"),
            #       paste(c("5 %:","50 %:","95 %:"),
            #       round(quantile(log10(Qplus[,h]),c(0.05,0.5,0.95),names=FALSE,na.rm=TRUE),2) ))
      ...
      if(input_modelchoice == "Dependent days"){
        ...
        for(u in 1:nU){ # for nU parameter sets
          ...
          if(ceiling(u/5)==floor(u/5)){ 
            thin<-thin+1 
            #if(nplus[u,h]<=19){                # initialized as NA
            #  exposurevarsample[thin,1]<-NA  
            #}
            if(nplus[u,h]>19){
              exposurevarsample[thin,1:nplus[u,h]] <- t(acuteexpopos[acuteexpopos[1:nV,h]>0,h]) 
