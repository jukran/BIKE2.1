                                           ...
                                           wellPanel( 
                                             ...
                                             fluidRow(
                                               column(
                                                 ...
                                                 radioButtons(
                                                   "modelchoice3",
                                                   label = h5(("Serving sizes"), style = "color:#343841"),
                                                   choices = c("No", "Yes")
                                                 )
                                               ),
                                               column(
                                                 7, radioButtons(
                                                   "modelchoice4",
                                                   label = h5(("Mean serving sizes"), style ="color:#343841"),
                                                   choices = c("No", "Yes")
                                                 )
                                               ))
                                           ),
...
does not lead to extremely small or large numerical values since this could affect also the numerical computations.
                                            Therefore, sensible measurement units have to be selected."
                                                              ...
                                                              tabPanel(
                                                                ...
                                                                tags$br(),
                                                                tags$p("The data file with food consumption has to contain
at least the columns with the following names: ", tags$b("IDnum, Weight, foodA1, foodA2, foodB1, foodB2, etc.")),
                                                                tags$ul(
...
grams per day. Note that the chosen weight units should match the units used for hazard
...
units.")),
                                                                tags$p("The number of reporting days must be the same for all persons. Missing values not allowed.
Consumption data file may also contain other columns although these are
not currently used in the model. For example, age of respondents."
                                                                ),
                                                                
                                                                tags$br(),
                                                                wellPanel(style = "background-color: #EEEEEE;",
                                                                          tags$p(
                                                                            "Food consumption data corresponds to food diary data format
...
etc. These columns would be followed by the same list of food types for the second day, e.g. ",
                                                                            tags$b("broiler2,
fish2, apple2,"),
                                                                            "etc. There need to be at least two days recorded for each consumer and the same
number of days for all. Each row represents the reported consumptions of one consumer. The food
...
this should be marked as", tags$b('NA'), ".")),
                                                                tags$p("Note that 'all' for concentration information implies that the concentration distribution will be
estimated jointly with prevalence parameter using a zero-inflated model where the fraction of
measurements below LOD are interpreted allowing the possibility of true zeros. Then, prevalence
data file should mark the corresponding hazard sample data as 'NA' to signify the hazard
prevalence is not estimated from separate sample information."),
...
specifying the name of the hazard (e.g. 'cadmium'), the type (",
                                                                            ...
                                                                            tags$b("'M'"),
                                                                            "for
microbiological), and the exposure limit. The remaining columns have headers according to the food types, and the row entry
...
zeros and small positive values when below LOD."),
                                                                tags$br(),
                                                                wellPanel(style = "background-color: #EEEEEE;",
                                                                          tags$p(
...
resampling of data (without replacement) to create artificial random replicate of data, with
original sample size."),
                                           tags$br(),
                                           tags$li(tags$b(" 2D simulation"),"
simulation of parameter values from uncertainty distribution (here by MCMC) and simulation
of variable quantities from variability distributions (which are defined by parameters)."),
                                           tags$br(),
...
proportion of contaminated food items."
                                           ...
                                           tags$li("LB substitution "),
                                           tags$li("UB substitution "),
                                           tags$li("posterior distribution ")
                                         )
                                       )
                              ),
...
server <- function(input, output, session) {
  ...
  solvedBugs <- eventReactive(input$run, {
    ...
    if (nf==1) {
      ...
      } else {   # ADD: delete NULL-definition because logitp0 always defined
        # logitp0_s = NULL
        Tp_s = NULL
        Sp_s = NULL
      }
